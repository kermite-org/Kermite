...
function convertHexStringSequenceToBytes(str: string): number[] {
  if (str.length % 2 !== 0) {
...
function createHexRecord(line: string): IHexRecord {
  ...
  const bytes = convertHexStringSequenceToBytes(lineBodyStr);
  ...
  const checksumCalculated = calcChecksum(bytes.slice(0, bytes.length - 1));
