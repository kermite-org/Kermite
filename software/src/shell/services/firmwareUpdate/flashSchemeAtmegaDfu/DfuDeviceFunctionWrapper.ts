...
import { DfuRequestType } from '~/shell/services/firmwareUpdate/flashSchemeAtmegaDfu/DfuConstants';
...
export function createDfuDeviceFunctionWrapper(
  ...
  const asyncControlTransfer = usbPromisify(
  ...
  async function requestRead(
    ...
    const buffer = (await asyncControlTransfer(
