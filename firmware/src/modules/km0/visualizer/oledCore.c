...
void oledCore_graphics_drawLine(int x0, int y0, int x1, int y1) {
  ...
  // __gr_putPixel(x1, y1);
  for (int i = 0; i < dd; i++) {
    oledCore_graphics_putPixel(px >> 8, py >> 8);
    px += ax;
    py += ay;
  }
}
...
void oledCore_graphics_drawFullImage(const uint32_t *pLineBuffers128) {
  for (int i = 0; i < 128; i++) {
    scanLines[i] = romData_readDoubleWord(pLineBuffers128 + i);
  }
}
...
void oledCore_graphics_setFontData(const uint8_t *_fontDataPtr, int _fontWidth, int _fontLetterSpacing) {
...
static void drawChar(int caretY, int caretX, char chr) {
  ...
  for (int i = 0; i < fontWidth; i++) {
    ...
    ptr[caretY] = romData_readByte(&fontDataPtr[fontIndexBase + i]);
  }
}
