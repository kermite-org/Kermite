...
static void pinStateChangedHanderRaw(uint pin, uint32_t events) {
...
void pinObserver_observePin(int pin, void (*callback)(int, int)) {
  ...
  gpio_set_irq_enabled_with_callback(pin, GPIO_IRQ_EDGE_RISE | GPIO_IRQ_EDGE_FALL, true, &pinStateChangedHanderRaw);
}
